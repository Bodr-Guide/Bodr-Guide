...
function PreparationContent({
  ...
  const cards: PrepCard[] = [
    ...
    (() => {
      const info = countryTravelInfo["JP"];
        ...
        {(() => {
          ...
          return (
            <div className="space-y-3">
              ...
              {/* 영사콜센터 */}
              <div className="p-3 bg-sky-50 dark:bg-sky-950/30 border border-sky-200/50 dark:border-sky-800/30 rounded-lg">
                <p className="text-xs text-sky-600 dark:text-sky-400 font-medium mb-1">영사콜센터 (24시간)</p>
                <a href="tel:+82-2-3210-0404" className="text-sm font-bold text-sky-800 dark:text-sky-200 hover:underline">+82-2-3210-0404</a>
              </div>

              {/* 외교부 */}
              <a href="https://www.0404.go.kr" target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 p-2.5 bg-slate-100 dark:bg-slate-800 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                외교부 해외안전여행 →
              </a>
            </div>
          );
